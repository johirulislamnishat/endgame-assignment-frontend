...
const ManageTestData = ({ lab }) => {
  const { _id, testName, testType, testPrice } = lab;
  ...
  const handleDeleteTest = (id) => {
    ...
    if (proceed) {
      const url = `https://rocky-mountain-61787.herokuapp.com/lab/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
    ...
    <TableRow hover role="checkbox" tabIndex={-1}>
      ...
      <TableCell style={{ fontSize: 20 }}>
        ...
        <button
          ...
          onClick={() => handleDeleteTest(lab._id)}
        >
          <svg
